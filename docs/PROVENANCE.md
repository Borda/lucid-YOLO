...
| R21 | fuse-augmentations, github.com/Borda/fuse-augmentations, commit 0a0cc64072b854936f3a15c36c21b5ed37012891, Apache-2.0 | Synthetic scene generator for test fixtures and dataset stand-ins (A26), pinned via git commit (WP-131) rather than a PyPI version since the version string does not move release to release. Advanced from commit 74af9855 to 0a0cc640 by WP-132, which needs that commit's `SymbolShape` family — the keypoint-bearing shapes the pose overfit gate draws. That bump is not fixture-neutral: the same commit redefines `GeomShape.TRIANGLE` from equilateral to obtuse-scalene and re-centres every outline on its area centroid, moving the det/obb/seg fixtures' own instance counts and box sums (see WP-132's log for the measured deltas and what was re-frozen). No YOLO-implementation lineage; verified Apache-2.0 |

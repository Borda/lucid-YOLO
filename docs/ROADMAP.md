...
| 046 | `release: v0.1.0 detector` [HUMAN] | O3 cleared; CHANGELOG; goldens frozen to `0.1`. No trained weights are published (D14) | `release.yml` green on tag `v0.1.0` | 045 | ⬜ |
...
| 087 | `feat(ptl): segmentation training path` | **Phase 7 built every segmentation component standalone and wired none into training**, so `task="segment"` trains the detection objective while the prototype, coefficient and semantic heads never receive a gradient — and every gate stays green, because each gate tests those components in isolation and each is correct there. Nine parts, all verified as missing rather than assumed: (1) `DetectionLitModule` composes backbone/neck/head itself (`module.py:245`), duplicating `Detector`; attaching seg branches there would create a second `Segmenter` that the Table S9 gate does not measure, so composition moves behind `build.py` and a test pins the module's state-dict keys and parameter count to `Segmenter`'s. (2) `_task_extra_loss` is still WP-034's inert zero. (3) `_rasterize_polygon` already exists private in `mixup.py:86` (even-odd rule, half-open blocks) — promote it rather than write a second rasterizer, or copy-paste and the mask loss disagree about which pixels a polygon covers. (4) Instance-mask targets rasterized at proto resolution in the augmented frame. (5) Targets gathered by TAL's `gt_index`, so mask supervision follows the same positives the box loss uses. (6) A dense per-class semantic target for the A17 branch. (7) Mask and auxiliary loss gains — unspecified by the papers, to be registered as **A38**. (8) `seg_tier_a_n.yaml`. (9) segm mAP in `validation_step` beside the WP-077 bbox mAP. The data path is **not** a gap: `PackedTargets` already carries polygon rings and all four augment modules transform them. **Prerequisite for WP-054's Seg-A run** | `test_seg_training.py`; module composition identical to `Segmenter`; overfit-100 mask IoU >= 0.7 **before** any COCO run — wrong wiring shows in minutes on 100 images and in hours on COCO | 053 | ⬜ |
...
## Phase 9 — Inference and generalization (WP-089…091, 074…075)

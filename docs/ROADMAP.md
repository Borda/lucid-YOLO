...
| 052b | `test(models): Table S9 seg fidelity gate` | Params/FLOPs vs R1 Table S9, all 5 scales, ±2%/±5%. **Blocked**: the published s/m/l rows are not in the repo and the arXiv HTML does not serve section S5; awaiting the three values. Measured today — n 2.717 M / 9.00 G against a published 2.7 M / 9.1 G (+0.6%, -1.1%) and x 61.900 M / 324.58 G against 62.8 M / 313.5 G (-1.4%, +3.5%) — so both known endpoints already sit inside tolerance | `test_param_flops.py::test_seg_vs_tableS9` — **golden frozen** | 052 | ⬜ |

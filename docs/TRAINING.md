...
The default is off because the claim that matters is not yet made: whether the compiled objective trains to the same mAP as eager is A78, and its validation — one `det_nano_smoke` epoch plus validation, `compile_step` true against false, `≤ 3` recompiles, step time `≥ x1.15` — has not been run. D24 names what flips the default.

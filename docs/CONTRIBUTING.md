...
| **Provenance trailers** | `WP:`/`Provenance:`/`Assumptions:`/`Gate:`, validated by a job in this repository | What an algorithmic change is derived from, in a form a reviewer can check against the register | Nothing about your right to submit |
...
The provenance job skips a commit an automation authored on the same reasoning: a pin bump has no derivation to name, nobody wrote the message, and what lands on `main` from such a pull request is the maintainer's squash-merge message, written by a person. The skip is printed by commit, so the verdict says what it left out, and a human commit in the same pull request is still checked on its own.

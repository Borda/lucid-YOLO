...
What is claimed is a faithful reproduction of the *method*, not of the paper's exact numbers ([decision record, D2](DECISIONS.md)). Each of the four models has been trained once, on a single device, at the smallest of five scales, on a deliberately short schedule — a smoke tier, in this project's vocabulary, sized to prove the mechanism works rather than to compete with a published leaderboard. No run has ever spanned more than one accelerator, so no distributed strategy is exercised or tuned for anywhere in this repository. No trained weights are published ([decision record, D14](DECISIONS.md)); the source, the frozen golden metrics, and the report below are the release artifacts.
...
| contribute a change | [Contributor guide](CONTRIBUTING.md) | the clean-room rule, DCO sign-off, provenance trailers, and what the gate does and does not decide |

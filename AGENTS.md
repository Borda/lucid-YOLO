...
- **`scripts/lint/` holds every repo-content checker that used to run inline as a `tests/meta/` pytest test** (licence audit, commit trailers, doctest coverage, flat test groups, docs presence, docs site, figure captions, license headers, version single-source): a `main(argv)` CLI plus a `.pre-commit-config.yaml` local hook. Every hook carries a distinct emoji in its `name:` and is individually callable — `pre-commit run <hook-id>` (e.g. `pre-commit run license-audit`) — without running the rest of the suite, which is also how a WP-scoped fix is verified before `make gate`. `golden-check` (`scripts/check_goldens.py`) and `release-guard` (`scripts/release_guard.py`) run on every commit and `pre-commit run --all-files` (`stages: [pre-commit, manual]`) as well as standalone (`pre-commit run --hook-stage manual golden-check`) — both are cheap enough off a release tag that always-run beats a second exemption to remember. `commit-trailers` (`stages: [commit-msg]`) validates the message actually being written and cannot run under `--all-files` at all, since no commit is in progress then; `commit-trailers-history` is its `--all-files`-reachable sibling, re-validating every commit ahead of `origin/main` via the same script's `--range` mode.

...
- **`scripts/lint/` holds every repo-content checker that used to run inline as a `tests/meta/` pytest test** (licence audit, commit trailers, doctest coverage, docs presence, docs site, figure captions, license headers, version single-source): a `main(argv)` CLI plus a `.pre-commit-config.yaml` local hook. Every hook is individually callable — `pre-commit run <hook-id>` (e.g. `pre-commit run license-audit`) — without running the rest of the suite, which is also how a WP-scoped fix is verified before `make gate`. Two hooks carry `stages: [manual]` rather than running on every commit: `golden-check` (`scripts/check_goldens.py`, already the expensive comparison `make golden` exists for) and `release-guard` (`scripts/release_guard.py`, release-tag time only) — defined as hooks for the same individually-callable uniformity, invoked explicitly (`pre-commit run --hook-stage manual golden-check`) rather than on every commit.

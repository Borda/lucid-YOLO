...
- the **one-to-one** (o2o) branch trains with a *unique* assignment — a
  :class:`~open_yolos.assign.one_to_one.UniqueAssigner` (``topk = 7`` candidate
...
:class:`~open_yolos.losses.detection_loss.DetectionBranchLoss` (both sharing the
...
from open_yolos.assign.stal import SmallTargetAssigner
...
class DualLossOutput:
    ...
    :class:`~open_yolos.losses.detection_loss.DetectionLossOutput` (its pre-gain
...
class DualBranchLoss:
    ...
    Holds two :class:`~open_yolos.losses.detection_loss.DetectionBranchLoss`
    instances (identical gains), a dense o2m assigner
    (:class:`~open_yolos.assign.stal.SmallTargetAssigner`, ``topk = 10``), and a
    unique o2o assigner (:class:`~open_yolos.assign.one_to_one.UniqueAssigner`,
    ...
    frame (see :class:`~open_yolos.losses.detection_loss.DetectionBranchLoss`).
    ...
    def __call__(
        ...
        Args:
            ...
            anchor_points: ``(A, 2)`` anchor-centre ``(x, y)`` pixels shared by
                both branches (see :func:`open_yolos.assign.make_anchor_points`).
        ...
        Returns:
            ...
            :class:`~open_yolos.losses.detection_loss.DetectionLossOutput`, and the
            ``alpha`` used.

...
``xyxy`` boxes with :func:`~open_yolos.models.heads.detect.decode_ltrb`, reduce to
the score-ranked detections with
:func:`~open_yolos.models.heads.detect.o2o_topk`, pad to a **fixed** ``(B, 300,
...
from open_yolos.decode.common import SCORE_COLUMN, pad_detections
from open_yolos.models.heads.detect import decode_ltrb, o2o_topk
...
class TopKDecoder(nn.Module):
    ...
    def forward(self, cls_logits: Tensor, raw_ltrb: Tensor, anchor_points: Tensor, strides: Tensor) -> Tensor:
        ...
        Args:
            ...
            anchor_points: Anchor-centre ``(x, y)`` coordinates of shape
                ...
                :func:`~open_yolos.assign.grid.make_anchor_points`.
    ...
    def _pad_to_k(self, detections: Tensor) -> Tensor:
        ...
        shared :func:`~open_yolos.decode.common.pad_detections` so both decode

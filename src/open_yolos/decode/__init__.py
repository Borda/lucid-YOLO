...
- :class:`~open_yolos.decode.topk_e2e.TopKDecoder` — the suppression-free
  ...
  sec. 4, A9);
- :class:`~open_yolos.decode.nms_path.NMSDecoder` — the non-E2E path over the dense
  one-to-many branch: confidence threshold then class-wise NMS, for the "mAP
  (non-E2E)" comparison column (WP-042; R1 sec. 3.2.1).

:func:`~open_yolos.decode.common.to_letterboxed_original` is the eval-time hook that
un-letterboxes either path's boxes back to original-image coordinates (A10).
"""

from open_yolos.decode.common import to_letterboxed_original
from open_yolos.decode.nms_path import NMSDecoder
from open_yolos.decode.topk_e2e import TopKDecoder

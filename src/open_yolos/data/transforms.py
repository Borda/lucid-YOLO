...
:class:`GeometricTransform`: it maps a CHW image and its :class:`~open_yolos.data.targets.Targets`
...
class GeometricTransform(Protocol):
    ...
    every modality carried by :class:`~open_yolos.data.targets.Targets` stays
    geometrically consistent with the returned image.

    Examples:
        ```pycon
        >>> from open_yolos.data.transforms import Compose, GeometricTransform

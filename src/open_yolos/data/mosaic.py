...
Not a :class:`~open_yolos.data.transforms.GeometricTransform`:
...
Targets:
    ...
    criteria :class:`~open_yolos.data.affine.RandomAffine` applies, kept local here
    ...
    :meth:`~open_yolos.data.targets.Targets.concat`, which requires consistent
...
from open_yolos.data.transforms import boxes_from_polygons
...
class MosaicAssembly:
    ...
    :meth:`~open_yolos.data.targets.Targets.concat`.

    This is an **assembly, not** a :class:`~open_yolos.data.transforms.GeometricTransform`:
    ...
    def __call__(self, items: list[tuple[Tensor, Targets]]) -> tuple[Tensor, Targets]:
        ...
        Args:
            items: Exactly four ``(image, targets)`` pairs. Each ``image`` is a CHW
                ...
                requirement of :meth:`~open_yolos.data.targets.Targets.concat`).

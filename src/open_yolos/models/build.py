...
Assembles the WP-020…WP-022 components (:class:`~open_yolos.models.backbone.DetectionBackbone`,
:class:`~open_yolos.models.neck.DetectionNeck`, :class:`~open_yolos.models.heads.DualDetectionHead`)
...
multipliers come from :func:`~open_yolos.models.registry.scale_spec`.
...
from open_yolos.models.heads import DualDetectionHead, DualHeadOutput
...
class Detector(nn.Module):
    ...
    :class:`~open_yolos.models.heads.DualHeadOutput` (both the one-to-one and
    one-to-many dense predictions).

    Args:
        variant: Scale name (``"n"``/``"s"``/``"m"``/``"l"``/``"x"``) resolved
            through :func:`~open_yolos.models.registry.scale_spec`.
    ...
    def forward(self, image: Tensor) -> DualHeadOutput:
        ...
        Returns:
            The head's :class:`~open_yolos.models.heads.DualHeadOutput` with dense
...
class _TupleOutputAdapter(nn.Module):
    ...
    :class:`~open_yolos.models.heads.DualHeadOutput`. This adapter unpacks such an
    ...
    def forward(self, image: Tensor) -> tuple[Tensor, ...] | Tensor:
        ...
        Returns:
            ...
            :class:`~open_yolos.models.heads.DualHeadOutput`, otherwise unchanged.
        """
...
def count_flops(module: nn.Module, img_size: int = _DEFAULT_IMG_SIZE) -> float:
    ...
    :class:`~open_yolos.models.heads.DualHeadOutput` dataclass, so a dataclass-output

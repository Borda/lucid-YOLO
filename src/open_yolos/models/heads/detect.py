...
:func:`~open_yolos.assign.grid.make_anchor_points` position-for-position, so the
...
from open_yolos.models.blocks import ConvBNAct, DepthwiseConv
...
def _flatten_level(feature_map: Tensor) -> Tensor:
    ...
    :func:`~open_yolos.assign.grid.make_anchor_points` so anchor row ``a`` owns
    prediction row ``a``.
...
class DualHeadOutput:
    ...
    :func:`~open_yolos.assign.grid.make_anchor_points`). Class fields are raw

...
dual detection head) with the WP-028 :class:`~open_yolos.losses.dual_loss.DualBranchLoss`
and the WP-032 :class:`~open_yolos.optim.musgd.MuSGD` optimizer into the Phase 5
...
Forward and loss wiring:
    ...
    :func:`~open_yolos.models.heads.detect.decode_ltrb`, pads the ragged
...
Progressive-loss schedule (WP-035):
    ...
    from a :class:`~open_yolos.losses.progressive.ProgressiveLossSchedule` — the
...
from open_yolos.losses.progressive import ProgressiveLossSchedule
from open_yolos.models.backbone import DetectionBackbone
from open_yolos.models.heads.detect import DualDetectionHead, DualHeadOutput, decode_ltrb
...
def pad_targets(targets: list[Targets]) -> tuple[Tensor, Tensor, Tensor]:
    ...
    The batch's :class:`~open_yolos.data.targets.Targets` each carry their own
    ...
    :class:`~open_yolos.losses.dual_loss.DualBranchLoss` assigners consume. An image
...
class DetectionLitModule(LightningModule):
    ...
    Composes :class:`~open_yolos.models.backbone.DetectionBackbone`,
    :class:`~open_yolos.models.neck.DetectionNeck`, and
    :class:`~open_yolos.models.heads.detect.DualDetectionHead` (built from the same
    compound-scaling multipliers), scores their dense predictions with
    :class:`~open_yolos.losses.dual_loss.DualBranchLoss`, and optimizes under
    Lightning automatic optimization (D4) with
    :class:`~open_yolos.optim.musgd.MuSGD`. See the module docstring for the A8
    ...
    def on_train_epoch_start(self) -> None:
        ...
        Sets :attr:`alpha` to
        :meth:`~open_yolos.losses.progressive.ProgressiveLossSchedule.alpha_at`
    ...
    def forward(self, images: Tensor) -> DualHeadOutput:
        ...
        Returns:
            The :class:`~open_yolos.models.heads.detect.DualHeadOutput` dense
    ...
    def configure_optimizers(self) -> MuSGD:
        ...
        Returns:
            A :class:`~open_yolos.optim.musgd.MuSGD` over ``self.parameters()`` built

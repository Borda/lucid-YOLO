...
- :func:`detections_to_predictions` turns a fixed-size ``(B, 300, 6)`` A9
  ...
  :class:`~open_yolos.decode.topk_e2e.TopKDecoder` and
  :class:`~open_yolos.decode.nms_path.NMSDecoder`) into torchmetrics prediction
...
from open_yolos.decode.common import BOX_CORNERS, SCORE_COLUMN, to_letterboxed_original
...
def detections_to_predictions(
    ...
    Args:
        ...
        label_to_category: Mapping from contiguous class label to original COCO
            category id (e.g.
            :attr:`~open_yolos.data.coco.CocoDetectionDataset.label_to_category_id`).
...
class DualPathEvaluator:
    ...
    **once**, both the suppression-free E2E path
    (:class:`~open_yolos.decode.topk_e2e.TopKDecoder` over the one-to-one branch)
    and the non-E2E path (:class:`~open_yolos.decode.nms_path.NMSDecoder` over the
    dense one-to-many branch) are decoded from the same
    :class:`~open_yolos.models.heads.detect.DualHeadOutput`, each un-letterboxed to
    ...
    Args:
        model: Any module whose forward maps ``(B, 3, H, W)`` images to a
            :class:`~open_yolos.models.heads.detect.DualHeadOutput` (e.g.
            :class:`~open_yolos.ptl.module.DetectionLitModule` or a bare
        ...
        letterbox: The validation :class:`~open_yolos.data.letterbox.Letterbox`

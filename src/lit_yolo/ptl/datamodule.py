...
Multi-image augmentation composition:
    ...
    ordinary single-image :class:`~lit_yolo.data.transforms.GeometricTransform` s
    chained in a :class:`~lit_yolo.data.transforms.Compose`. They are composed
    ...
    single seeded :class:`torch.Generator` and assembles the sample:
        ...
        2. with probability ``mixup`` a second full geometric sample is blended in
           via :class:`~lit_yolo.data.mixup.Mixup`;
        3. with probability ``copy_paste`` polygon instances from a third geometric
           sample are pasted in via :class:`~lit_yolo.data.mixup.CopyPaste`;
        4. photometric :class:`~lit_yolo.data.augment.HSVJitter` and
           :class:`~lit_yolo.data.augment.HorizontalFlip` finish the sample.

    The per-sample mixup/copy-paste probabilities come from
    :func:`~lit_yolo.data.coco.build_scale_policy` (size-aware, [R1] Table S3);
...
Batch contract:
    ...
    :class:`~lit_yolo.data.targets.Targets` **as a list** of length ``B`` — targets
...
from lit_yolo.data.affine import RandomAffine
from lit_yolo.data.augment import HorizontalFlip, HSVJitter
...
def collate_detection(batch: list[tuple[Tensor, Targets]]) -> tuple[Tensor, list[Targets]]:
    ...
    into one dense tensor; the per-image :class:`~lit_yolo.data.targets.Targets`
    ...
    Returns:
        ...
        ``targets`` is a length-``B`` list of :class:`~lit_yolo.data.targets.Targets`.
...
class _TrainPipeline(Dataset[tuple[Tensor, Targets]]):
    ...
    Args:
        ...
        policy: The size-aware strengths from
            :func:`~lit_yolo.data.coco.build_scale_policy` (``scale``, ``mixup``,
            ``copy_paste``).

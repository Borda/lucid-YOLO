...
:mod:`scripts.check_data` and :class:`~lit_yolo.data.coco.CocoDetectionDataset`
already expect::
...
Examples:
    Fetch the validation split plus annotations::
        ...
        python -m lit_yolo.data.download --data-root /data/coco

    Or via the console script (both splits)::

        lit-yolo-download --data-root /data/coco --splits train val
"""
...
def _build_parser() -> argparse.ArgumentParser:
    """Build the ``lit-yolo-download`` argument parser."""

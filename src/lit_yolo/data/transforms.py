...
:class:`GeometricTransform`: it maps a CHW image and its :class:`~lit_yolo.data.targets.Targets`
...
class GeometricTransform(Protocol):
    ...
    every modality carried by :class:`~lit_yolo.data.targets.Targets` stays
    geometrically consistent with the returned image.

    Examples:
        ```pycon
        >>> from lit_yolo.data.transforms import Compose, GeometricTransform

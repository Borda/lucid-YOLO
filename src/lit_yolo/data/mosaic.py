...
Not a :class:`~lit_yolo.data.transforms.GeometricTransform`:
...
Targets:
    ...
    criteria :class:`~lit_yolo.data.affine.RandomAffine` applies, kept local here
    ...
    :meth:`~lit_yolo.data.targets.Targets.concat`, which requires consistent
...
from lit_yolo.data.transforms import boxes_from_polygons
...
class MosaicAssembly:
    ...
    :meth:`~lit_yolo.data.targets.Targets.concat`.

    This is an **assembly, not** a :class:`~lit_yolo.data.transforms.GeometricTransform`:
    ...
    def __call__(self, items: list[tuple[Tensor, Targets]]) -> tuple[Tensor, Targets]:
        ...
        Args:
            items: Exactly four ``(image, targets)`` pairs. Each ``image`` is a CHW
                ...
                requirement of :meth:`~lit_yolo.data.targets.Targets.concat`).

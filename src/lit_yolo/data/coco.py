...
:class:`~lit_yolo.data.targets.Targets` stay in lock-step.
...
Crowd / RLE policy:
    ...
    exactly one ring, so :class:`~lit_yolo.data.targets.Targets` sees a consistent
...
if TYPE_CHECKING:
    ...
    from lit_yolo.data.transforms import GeometricTransform
...
#: ``scale`` is the :class:`~lit_yolo.data.affine.RandomAffine` half-range, ``mixup``
...
class CocoDetectionDataset(Dataset[tuple[Tensor, Targets]]):
    ...
    :class:`~lit_yolo.data.transforms.GeometricTransform`, e.g.
    :class:`~lit_yolo.data.letterbox.Letterbox` for validation) is applied to each
    ...
    def __getitem__(self, index: int) -> tuple[Tensor, Targets]:
        ...
        Returns:
            A CHW float32 image in ``[0, 1]`` and its :class:`~lit_yolo.data.targets.Targets`
    ...
    def _build_targets(self, record: _ImageRecord) -> Targets:
        """Assemble the :class:`~lit_yolo.data.targets.Targets` for one image."""

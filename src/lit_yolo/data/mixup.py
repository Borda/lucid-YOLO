...
:class:`~lit_yolo.data.transforms.GeometricTransform` implementations: like
:class:`~lit_yolo.data.mosaic.MosaicAssembly` consumes four ``(image, targets)``
...
class Mixup:
    ...
    consistent polygon presence (a :meth:`~lit_yolo.data.targets.Targets.concat`
    rule).
    ...
    def __call__(self, items: list[tuple[Tensor, Targets]]) -> tuple[Tensor, Targets]:
        ...
        Args:
            items: Exactly two ``(image, targets)`` pairs. The two images must share
                ...
                :meth:`~lit_yolo.data.targets.Targets.concat` requirement).
...
class CopyPaste:
    ...
    **source** ``b``. For each polygon-carrying instance of ``b`` a paste draw is
    ...
    :class:`~lit_yolo.data.targets.Targets` rejects. This matches the segmentation

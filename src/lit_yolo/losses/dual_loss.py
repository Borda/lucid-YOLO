...
- the **one-to-one** (o2o) branch trains with a *unique* assignment — a
  :class:`~lit_yolo.assign.one_to_one.UniqueAssigner` (``topk = 7`` candidate
...
:class:`~lit_yolo.losses.detection_loss.DetectionBranchLoss` (both sharing the
...
from lit_yolo.assign.stal import SmallTargetAssigner
...
class DualLossOutput:
    ...
    :class:`~lit_yolo.losses.detection_loss.DetectionLossOutput` (its pre-gain
...
class DualBranchLoss:
    ...
    Holds two :class:`~lit_yolo.losses.detection_loss.DetectionBranchLoss`
    instances (identical gains), a dense o2m assigner
    (:class:`~lit_yolo.assign.stal.SmallTargetAssigner`, ``topk = 10``), and a
    unique o2o assigner (:class:`~lit_yolo.assign.one_to_one.UniqueAssigner`,
    ...
    frame (see :class:`~lit_yolo.losses.detection_loss.DetectionBranchLoss`).
    ...
    def __call__(
        ...
        Args:
            ...
            anchor_points: ``(A, 2)`` anchor-centre ``(x, y)`` pixels shared by
                both branches (see :func:`lit_yolo.assign.make_anchor_points`).
        ...
        Returns:
            ...
            :class:`~lit_yolo.losses.detection_loss.DetectionLossOutput`, and the
            ``alpha`` used.

...
Assembles the WP-020…WP-022 components (:class:`~lit_yolo.models.backbone.DetectionBackbone`,
:class:`~lit_yolo.models.neck.DetectionNeck`, :class:`~lit_yolo.models.heads.DualDetectionHead`)
...
multipliers come from :func:`~lit_yolo.models.registry.scale_spec`.
...
from lit_yolo.models.heads import DualDetectionHead, DualHeadOutput
...
class Detector(nn.Module):
    ...
    :class:`~lit_yolo.models.heads.DualHeadOutput` (both the one-to-one and
    one-to-many dense predictions).

    Args:
        variant: Scale name (``"n"``/``"s"``/``"m"``/``"l"``/``"x"``) resolved
            through :func:`~lit_yolo.models.registry.scale_spec`.
    ...
    def forward(self, image: Tensor) -> DualHeadOutput:
        ...
        Returns:
            The head's :class:`~lit_yolo.models.heads.DualHeadOutput` with dense
...
class _TupleOutputAdapter(nn.Module):
    ...
    :class:`~lit_yolo.models.heads.DualHeadOutput`. This adapter unpacks such an
    ...
    def forward(self, image: Tensor) -> tuple[Tensor, ...] | Tensor:
        ...
        Returns:
            ...
            :class:`~lit_yolo.models.heads.DualHeadOutput`, otherwise unchanged.
        """
...
def count_flops(module: nn.Module, img_size: int = _DEFAULT_IMG_SIZE) -> float:
    ...
    :class:`~lit_yolo.models.heads.DualHeadOutput` dataclass, so a dataclass-output

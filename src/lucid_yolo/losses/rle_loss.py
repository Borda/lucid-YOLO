...
class RLELoss(nn.Module):
    ...
    def forward(self, mu_hat: Tensor, sigma_raw: Tensor, mu_gt: Tensor, visibility: Tensor) -> Tensor:
        ...
        flow_log_prob = self.flow.log_density(residual.reshape(-1, 2)).reshape(residual.shape[:-1])  # (N, K)

        log_sigma_term = torch.log(sigma_hat).sum(dim=-1)  # (N, K)
        per_point = -base_log_prob - flow_log_prob + log_sigma_term  # (N, K), R14 Eq. 8 (log s dropped)

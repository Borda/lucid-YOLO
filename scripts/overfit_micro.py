...
:class:`~open_yolos.ptl.module.DetectionLitModule` is trained on a fixed ~100-image
...
:class:`~open_yolos.optim.musgd.MuSGD` optimizer, the progressive-loss ramp, and the
...
through the NMS-free :class:`~open_yolos.decode.topk_e2e.TopKDecoder` and every
...
from open_yolos.models.heads.detect import DualHeadOutput
...
#: read from the packaged configs tree (``open_yolos/configs``).
_RECIPE_PATH = Path(open_yolos.__file__).resolve().parent / "configs" / "overfit_100.yaml"
...
def build_datamodule(split: Path, recipe: Recipe) -> DetectionDataModule:
    ...
    Returns:
        A :class:`~open_yolos.ptl.datamodule.DetectionDataModule` with ``num_workers=0``
...
def build_module(recipe: Recipe, num_classes: int) -> DetectionLitModule:
    ...
    Returns:
        A :class:`~open_yolos.ptl.module.DetectionLitModule` at the recipe's scale.
...
def evaluate_recall(module: DetectionLitModule, datamodule: DetectionDataModule) -> tuple[float, int]:
    ...
    the NMS-free :class:`~open_yolos.decode.topk_e2e.TopKDecoder`, and greedily matches

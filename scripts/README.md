...
| `release_guard.py` | Release gate (WP-006): a candidate tag ships only if it's a shippable `v0.MINOR.PATCH` string (ADR-002: no 1.0, ever), its changelog section exists, and `make gate` is green. No-op (exit 0) when `HEAD` isn't exactly a tag. | Release WPs [PRINCIPAL]; also the `release-guard` pre-commit hook, every commit / `pre-commit run --all-files`, or standalone via `pre-commit run --hook-stage manual release-guard` | `_tests/test_release_guard.py` |
